...
<div style="display: flex; flex-flow: column; overflow: auto;">
    ...
    <div class="content">
        ...
        <div class="extras-container border" style="padding: 24px">
            <div class="item">
                ...
                <select class="control" style="height: auto; min-width: auto;" id="loggerLevel"></select>
            </div>

            <div class="item">
                <label style="margin-right: 8px;" for="wiFiMode">WiFi mode</label>
                <select class="control" style="height: auto; min-width: auto;" id="wiFiMode"></select>
            </div>

            <div class="item">
                <label style="margin-right: 8px; min-width: fit-content;" for="timezone">Time zone</label>
                <select class="control" style="height: auto; max-width: 200px;" id="timezone"></select>
            </div>
        ...
        <div class="border" style="padding: 24px">
            ...
            <div style="display: flex; flex-flow: row; align-items: center; flex-wrap: wrap;">
                ...
                <h5 class="item m-0">http://<span class="device-ip">192.168.4.1</span>/keystore?delay=DELAY&key1=DATA&key2=DATA&key3...</h5>
            </div>
        ...
        <div class="border" style="padding: 24px">
            ...
            <div style="display: flex; flex-flow: row; align-items: center; margin-left: 16px;">
                ...
                <h5 class="item m-0">http://<span class="device-ip">192.168.4.1</span>/external?led=${on/off}&beep={on/off}</h5>
            </div>
        </div>
        ...
        <div class="border" style="padding: 24px;">
            <div class="item">
                ...
                <select class="control" style="height: auto; min-width: auto;" id="statisticLevel"></select>
            </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); background: lightgray;">
        <div class="flexbox-wrap">
            ...
            <h5 style="margin: 5px" id="fwVersion"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">HW version:</h5>
            <h5 style="margin: 5px" id="hwVersion"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">MAC:</h5>
            <h5 style="margin: 5px" id="mac"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">IP:</h5>
            <h5 style="margin: 5px" id="ip"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">Free HEAP:</h5>
            <h5 style="margin: 5px" id="heap"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">Keystore keys:</h5>
            <h5 style="margin: 5px" id="ksKeys"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">Toggle state:</h5>
            <h5 style="margin: 5px" id="toggleState"></h5>
        </div>
        <div class="flexbox-wrap">
            <h5 style="color: #cb1d38; font-weight: 200; margin: 5px">WiFi mode:</h5>
            <h5 style="margin: 5px" id="wiFiModeStatus"></h5>
        </div>
        <div class="flexbox-wrap" style="align-items: center; gap: 4px">
            <div id="rFlag" class="rgb-info-square"><h5 style="margin: 5px; color: #e50c0c;">R</h5></div>
            <div id="gFlag" class="rgb-info-square"><h5 style="margin: 5px; color: #077307;">G</h5></div>
            <div id="bFlag" class="rgb-info-square"><h5 style="margin: 5px; color: #4949ff;">B</h5></div>
        </div>
    </div>
...
<script>
        ...
        try {
            ...
            if (eventsRes.ok) {
                ...
                if (eventsJson && eventsJson.result) {
                    eventDataObj = eventsJson.result || {};
                    populateTable(eventDataObj);
                }
            }
            ...
            console.error('Error loading settings, events, networks, or status:', e);
        }
    }
    ...
    function renderStatus(status) {
        ...
        if (document.getElementById('ip')) document.getElementById('ip').textContent = status.ip || '';
        ...
        if (document.getElementById('wiFiModeStatus')) document.getElementById('wiFiModeStatus').textContent = status.wiFiMode || '';
    ...
    function renderSettings(cfg) {
        ...
        const checkboxSettings = [
            'useDnsName', 'loggerEnabled', 'useSound', 'statisticEnabled',
    ...
    function populateTable(data) {
        ...
        for (const key in events) {
            const row = `
                        <tr ${colorMap[key.substring(0,3)] || ''}>
                          <td>${key}</td>
                          <td >${events[key]}</td>
                          <td style="display: ${isEditMode ? 'flex' : 'none'}; justify-content: flex-end; gap: 8px;">
        ...
        toggleActionsColumn(isEditMode);
    }
    ...
    function toggleActionsColumn(showControls) {
        isEditMode = Boolean(showControls);
        ...
        if (header) header.style.display = showControls ? '' : 'none';
